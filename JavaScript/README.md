...
let a = 2
```
여기서 이퀄(=)기호가 할당 연산자에 해당한다. 위에 변수를 썼던 것을 확인하면 const 는 값을 재할당이 불가능하고 let은 재할당이 가능하다는 것을 알 수 있다.<br/>
a라는 키워드에서 값을 재할당 하기 위해서는 let을 써야한다.
...
console.log(a)
```
a라는 키워드에 a(a=2) 와 1를 더하면 a라는 변수에 숫자 3를 재할당 하는 것이다.
```jsx
// 첫째 줄과 둘째 줄의 코드는 서로 같고, 다른 방식으로 나타낸 것이다. 
a = a + 1
a += 1
```